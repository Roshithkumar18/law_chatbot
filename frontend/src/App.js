...
const COLORS = ["#c084fc", "#34d399", "#fbbf24", "#f87171", "#818cf8", "#fb7185"];
...
function ChartRenderer({ chartData }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      {chartData.by_status?.length > 0 && (
        <div style={styles.chartBox}>
          <p style={styles.chartTitle}>📊 Cases by Status</p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.by_status}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2a2a3d" />
              <XAxis dataKey="status" stroke="#6b7280" tick={{ fontSize: 11, fill: "#9ca3af" }} />
      ...
      {chartData.by_type?.length > 0 && (
        <div style={styles.chartBox}>
          <p style={styles.chartTitle}>🍩 Cases by Type</p>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={chartData.by_type} dataKey="count" nameKey="type" cx="50%" cy="50%" outerRadius={75} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={{ stroke: "#6b7280" }}>
                {chartData.by_type.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
              <Tooltip contentStyle={styles.tooltip} />
            </PieChart>
          </ResponsiveContainer>
      ...
      {chartData.by_state?.length > 0 && (
        <div style={styles.chartBox}>
          <p style={styles.chartTitle}>📈 Top States by Crime</p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData.by_state} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#2a2a3d" />
              <XAxis type="number" stroke="#6b7280" tick={{ fontSize: 10, fill: "#9ca3af" }} />
              <YAxis dataKey="state" type="category" stroke="#6b7280" tick={{ fontSize: 10, fill: "#9ca3af" }} width={80} />
              <Tooltip contentStyle={styles.tooltip} />
...
function MermaidRenderer({ code, type }) {
  ...
  return (
    <div style={styles.chartBox}>
      ...
      <pre style={{ color: "#34d399", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", whiteSpace: "pre-wrap", lineHeight: 1.6, overflow: "auto", maxHeight: 280, margin: 0 }}>
        {code}
      </pre>
      <p style={{ color: "#4b5563", fontSize: 11, marginTop: 8 }}>
        💡 Render at <a href="https://mermaid.live" target="_blank" rel="noreferrer" style={{ color: "#c084fc" }}>mermaid.live</a>
      </p>
    </div>
  );
}
...
export default function App() {
  ...
  const WELCOME_MSG = {
    ...
    chart_data: null, line_data: null, mermaid: null, mermaid_type: null, sql_executed: null
  };
  ...
  async function handleSend() {
    ...
    const userMsg = {
      ...
      chart_data: null, line_data: null,
      ...
      <div style={{ ...styles.sidebar, width: sidebarOpen ? 280 : 0, overflow: "hidden" }}>
        <div style={styles.sidebarInner}>
          ...
          <div style={styles.historyList}>
            {sessions.length === 0 && (
              <div style={styles.emptyHistory}>No chats yet. Start a new conversation!</div>
            )}
            {sessions.map(session => (
              <div key={session.id}
                ...
                style={{
                  ...styles.historyItem,
                  background: activeSession === session.id ? "rgba(192, 132, 252, 0.15)" : "transparent",
                  borderColor: activeSession === session.id ? "rgba(192, 132, 252, 0.4)" : "transparent"
                }}>
      ...
      <div style={styles.main}>
        ...
        <div style={styles.topBar}>
          ...
          <div style={styles.topTitle}>
            {activeSession ? sessions.find(s => s.id === activeSession)?.title || "Chat" : "The Akashic Array"}
          </div>
                ...
                <div style={{ maxWidth: "78%" }}>
                  {msg.role === "assistant" && (
                    <div style={styles.assistantLabel}>
                      <span style={styles.assistantAvatar}>⚖️</span>
            ...
            {loading && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <div style={{ ...styles.bubble, background: "#1a1a2e", border: "1px solid #2a2a3d", borderRadius: "4px 18px 18px 18px", display: "flex", gap: 5, padding: "14px 18px" }}>
                  {[0, 1, 2].map(j => (
                    <div key={j} style={{ width: 7, height: 7, borderRadius: "50%", background: "#c084fc", animation: "pulse 1.2s infinite", animationDelay: `${j * 0.3}s` }} />
                  ))}
                </div>
              </div>
            )}
        ...
        {/* Suggestions strip when chat active */}
        {messages.length > 0 && (
        ...
        <div style={styles.inputArea}>
          <div style={styles.inputBox}>
            <input
              ...
              placeholder="Ask anything about Indian law, cases, IPC sections..."
              style={styles.input}
            />
            <button onClick={handleSend} disabled={loading} style={{
              ...styles.sendBtn,
              background: loading ? "#2a2a3d" : "linear-gradient(135deg, #7c3aed, #a855f7)",
